...
from SmilesDecoder import SmilesDecoder
from train_gene_vae import train_gene_vae, load_test_gene_data
from train_smiles_decoder import load_smiles_data, train_smiles_decoder
...
parser.add_argument('--saved_gene_vae', type=str, default='results/saved_gene_vae.pkl', help='Save the trained GeneVAE')
...
# ===========================
# SmilesDecoder
parser.add_argument('--train_smiles_decoder', action='store_true', help='Train SmilesDecoder') # Add --train_smiles_decoder to train SmilesDecoder
parser.add_argument('--test_smiles_decoder', action='store_true', help='Test SmilesDecoder') # Add --test_smiles_decoder to test SmilesDecoder
...
parser.add_argument('--saved_smiles_decoder', type=str, default='results/saved_smiles_decoder.pkl', help='Save the trained SmilesDecoder')
...
parser.add_argument('--smiles_decoder_train_results', type=str, default='results/smiles_decoder_train_results.csv', help='Path to save the results of trained SmilesDecoder')
...
def main(args):
    ...
    if args.train_gene_vae:
        ...
        trained_gene_vae.load_model(args.saved_gene_vae)
    ...
    if args.test_gene_vae:
        # Print SmilesDecoder hyperparameter information
        show_smiles_decoder_hyperparamaters(args)
    ...
    # ========================================================= #
    #                 3. Train SmilesDecoder                    #
    # ========================================================= #
    if args.train_smiles_decoder:
        # Train SmilesDecoder
        trained_smiles_decoder = train_smiles_decoder(
            trained_gene_vae,
            train_dataloder, 
            valid_dataloder,
            tokenizer, 
    ...
    if args.generation:
        ...
        # Load the trained SmilesDecoder  
        trained_smiles_decoder = SmilesDecoder(
            tokenizer,
        ...
        trained_smiles_decoder.load_model(args.saved_smiles_decoder)

        # Test mode
        trained_gene_vae.eval()
        trained_smiles_decoder.eval()
        ...
        for sd in range(args.random_seeds):
            ...
            for _, genes in enumerate(test_gene_loader):
                ...
                dec_sampled_char = trained_smiles_decoder.sample(args.max_len, latent_vectors)
